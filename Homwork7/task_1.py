...
class Segment:
    ...
    def length(self):
        return round(((self.x2 - self.x1) ** 2 + (self.y2 - self.y1) ** 2) ** (1. / 2.), 2)

    def x_axis_intersection(self):
        return (self.y1 > 0 and self.y2 < 0) or (self.y1 < 0 and self.y2 > 0)

    def y_axis_intersection(self):
        return (self.x1 > 0 and self.x2 < 0) or (self.x1 < 0 and self.x2 > 0)
